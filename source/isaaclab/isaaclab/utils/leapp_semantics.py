...
def resolve_leapp_element_names(semantics: LeappTensorSemantics | None, data_self) -> list | None:
    ...
    if source == "joint_names":
        return _select_element_names(getattr(data_self, "joint_names", None), getattr(data_self, "_joint_ids", None))
    if source == "body_names":
        return _select_element_names(getattr(data_self, "body_names", None), getattr(data_self, "_body_ids", None))
    if source == "body_pose":
        body_names = _select_element_names(
            getattr(data_self, "body_names", None), getattr(data_self, "_body_ids", None)
        )
